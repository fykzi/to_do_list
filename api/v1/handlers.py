...
@user_router.post("/create-user", response_model=User)
async def create_user(db: AsyncSession = Depends(get_db)) -> User:
...
async def get_tasks(user_id: str, db: AsyncSession = Depends(get_db)) -> List[ShowTask]:
    ...
    if not res:
        raise HTTPException(status_code=401, detail="Не удалоcь получить ваши задачи")
    return res
